...
public:
    ...
    int findNumberOfLIS(vector<int>& nums) {
        ...
        vector<vector<int>> count(n+1,vector<int>(n+1,1));
        vector<vector<int>> length(n+1,vector<int>(n+1,0)); 

        for(int idx=n-1;idx>=0;--idx){
            for(int prev=0;prev<=idx;++prev){ // move -1 to be 0 and so on
                int skipCnt = count[idx+1][prev]; 
                int skipLen = length[idx+1][prev];

                int takeCnt = 0, takeLen = 0;

                if(prev==0 || nums[idx]>nums[prev-1]){
                    takeCnt = count[idx+1][idx+1] ;
                    takeLen = length[idx+1][idx+1]+1;
                }

                length[idx][prev] = max(skipLen,takeLen) ;
                count[idx][prev] = (skipLen>takeLen) ? skipCnt : (takeLen>skipLen) ? takeCnt : skipCnt + takeCnt;
            }
        }
